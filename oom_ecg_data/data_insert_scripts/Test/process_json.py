...
print("Test process json")
class MongodbJobProcessor:
    def __init__(self):
      # remote ECG DB
        self.db_remote = pymongo.MongoClient(
            "mongodb://admin:KmtOom2023@191.169.1.6:27017/ecgs1?authSource=admin" #test db
        )["oom-ecg"]
      ...
      # local processed DB
        ...
        # job queue DB
        self.job_db = pymongo.MongoClient("mongodb://localhost:27017/")["test"]["Test2"]
        # self.job_db = pymongo.MongoClient("mongodb://192.168.1.65:27017/")["Queue"]["Test_Queue"]
    ...
    #   print("runnn")
    #   while True:
    #       job = self.job_db.find_one({"_status": "pending"})   # Look for pending jobs
    #       if job:
