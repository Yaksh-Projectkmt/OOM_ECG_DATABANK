...
async function checkPaidStatus(objectId) {
    try {
        const res = await fetch("/auth/check_paid_status/", {
            method: "POST",
            headers: {
        ...
        console.error("Paid status check failed:", err);
        return false;
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const selectors = {
    cards: '.card[data-arrhythmia], .condition-card[data-arrhythmia], .card-tile[data-arrhythmia]',
...
const attachRowEventListeners = () => {
  ...
  tableBody.addEventListener('click', debounce(async (e) => {
    ...
    const isPaid = await checkPaidStatus(objectId);

    // store globally (reuse later)
    if (!window.paidStatus) window.paidStatus = {};
    window.paidStatus[objectId] = isPaid;
    ...
    const samplesTaken = parseInt(row.getAttribute("data-samples-taken")) || 4000;
    ...
    if (!isVisible) {
      ...
      try {
        ...
        const paidBadgeEl = document.getElementById(`paidBadge-${objectId}`);
        if (paidBadgeEl) {
            paidBadgeEl.innerHTML = window.paidStatus?.[objectId]
          ...
          switch (downloadType) {
            ...
            //Deduct wallet BEFORE allowing download
              ...
              if (leadNumeric === '2') {
                ...
                const okRaw = await deductWallet("csv",patientId, arrhythmia, leadType,DownloadfileId,objectId);  
                  if (!okRaw) return;
            ...
            case 'plot_png':
                ...
                // Get FIGURE safely from Plotly (no circular JSON)
                ...
                const okPng = await deductWallet("image",patientId, arrhythmia, leadType,DownloadfileId,objectId);
                if (!okPng) return;
              ...
              const okPqrst = await deductWallet("csv",patientId, arrhythmia, leadType,DownloadfileId,objectId);
              if (!okPqrst) return;
              downloadCSV(csvContent, filename);
              updateWalletBalance();
              if (pageLoader) pageLoader.style.display = 'none';
              ...
              <!-- Share dropdown -->
                <div class="share-dropdown">
                  <i class="fas fa-share share-icon" 
                    ...
                    <span id="paidBadge-${row.object_id || ''}"></span>
...
window.plotECGPreview = async function (ecgData, leadType, leadNames, plotContainer) {
    ...
    const bgColor = isDarkMode ? '#1e2a44' : '#000000ff';
    const fontColor = isDarkMode ? '#fff' : '#fffdfdff';
...
async function deductWallet(fileType, patientId, arrhythmia, leadType,DownloadfileId,objectId) {
...
async function uploadFileToDB({ fileBlob, filename, patientId, DownloadfileId, downloadType, arrhythmia, leadType }) {
    const formData = new FormData();
