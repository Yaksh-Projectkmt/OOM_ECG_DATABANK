...
# Background Worker
# -------------------------
def background_job(job_dir, lead_type, job_id):
    try:
        final_pdf = run_ecg_st_pipeline(
            input_folder=job_dir,
            output_folder=job_dir,
        ...
        with open(final_pdf, "rb") as f:
            file_id = fs.put(
                ...
                filename=os.path.basename(final_pdf),
                contentType="application/pdf",
                metadata={"job_id": job_id, "lead_type": lead_type}
            )
...
def run_ecg_analysis(request):
    ...
    threading.Thread(
        ...
        args=(job_dir, lead_type, job_id),
        daemon=True
    ).start()

    # Return immediately (no timeout now)
    return JsonResponse({
        "status": "started",
        "job_id": job_id
    })
...
def job_status(request, job_id):
    return JsonResponse(JOB_STATUS.get(job_id, {"status": "unknown"}))
...
def view_pdf(request, file_id):
    try:
        ...
        response = FileResponse(grid_file, content_type="application/pdf")
        response["Content-Disposition"] = "inline; filename=ecg.pdf"
        return response
...
def download_pdf(request, file_id):
    ...
    response = FileResponse(pdf, content_type="application/pdf")
    response["Content-Disposition"] = f'attachment; filename="ST_Segment_{file_id}.pdf"'
    return response
