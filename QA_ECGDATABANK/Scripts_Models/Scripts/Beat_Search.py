...
try:
   R_PEAK_INTERPRETER = tf.lite.Interpreter(
       model_path =os.getenv("rnn_model")
   )
...
#        self.reference_r = list(reference_r) if reference_r else []
#
#        # AUTO detect R if missing (matches GUI)
#        if not self.reference_r:
#            r = np.argmax(np.abs(self.reference_beat))
#            self.reference_r = [r]
#
#        # REQUIRED for polarity gate inside is_similar_beat
#        self.reference_sig = self.reference_beat.copy()
#
...
class ECGBatchEngine:
    ...
    def __init__(self, folder, reference_beat, reference_idx,
                 reference_raw_full, reference_r,
                 reference_q, reference_s, terminology):
    ...
    def is_similar_beat(self, multi_sig_f, sig_f, start, beat_len):
        ...
        # R is already aligned in process_file()
        ref_r = self.reference_r[0]
        ref_offset = ref_r   # <<< FIX: DO NOT subtract reference_idx anymore
        ...
        ref = normalize(self.reference_beat.copy())
        win = normalize(sig_f[s:e].copy())
        ...
        score = 0.45 * core_corr + 0.35 * ext_corr + 0.20 * amp_score
    ...
    def process_file(self, file_path):
        ...
        for lead in df.columns:
            ...
            for r in r_peaks:
                ...
                # ALIGN DIRECTLY ON R (NO DOUBLE OFFSET)
                start = r - self.reference_r[0]

                ok, _, score = self.is_similar_beat(None, sig_f, start, beat_len)
