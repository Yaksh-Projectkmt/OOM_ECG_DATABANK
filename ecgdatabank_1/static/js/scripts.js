...
Plotly.purge(plotElement);
  ...
  // Main trace (empty for animation)
  const mainTrace = { x: [], y: [], mode: 'lines', name: 'ECG', line: { color: 'black', width: 1.5 } };
...
xaxis: {
  ...
  const config = { responsive: true, displaylogo: false, modeBarButtonsToAdd: ['pan2d', 'zoom2d', 'select2d', 'autoScale2d', 'resetScale2d'] };
  ...
  Plotly.newPlot(plotElement, [mainTrace], layout, config).then(() => {
    ...
    function animateWave() {
      if (i < dataLength) {
        const nextX = data.x.slice(i, i + frameStep);
        const nextY = scaledY.slice(i, i + frameStep);
        Plotly.extendTraces(plotElement, { x: [nextX], y: [nextY] }, [0]);
        i += frameStep;
        requestAnimationFrame(animateWave);
      } else {
        addPQRSTMarkers(); // Add markers after animation
      }
    }
    requestAnimationFrame(animateWave);

    // Add PQRST markers after animation
    function addPQRSTMarkers() {
      if (!pqrst) return;
      const markerSize = 8;
      const markerTraces = [];

      [['p_points','P','orange'], ['q_points','Q','blue'], ['r_peaks','R','red'], ['s_points','S','green'], ['t_points','T','purple']].forEach(([key,label,color]) => {
        if (pqrst[key]?.length) {
          markerTraces.push({
            ...
            ids: pqrst[key].map((_, idx) => `${key}-${idx}`)
          });
        }
      });
    ...
    // Enable PQRST dragging
    enablePQRSTEditing(plotElement, data, pqrst);

    // Selection for download (always full ECG)
    plotElement.on('plotly_selected', eventData => {
      if (!eventData?.points?.length) {
        window.selectedData = { x: [], y: [] };
        return;
      }

      const fullX = data.x;
      const fullY = data.y;
      const xVals = eventData.points.map(pt => pt.x);
      const minX = Math.min(...xVals);
      const maxX = Math.max(...xVals);

      const selectedIndices = fullX.map((v,i) => ({v,i})).filter(p => p.v >= minX && p.v <= maxX).map(p => p.i);

      window.selectedData = { x: selectedIndices.map(i => fullX[i]), y: selectedIndices.map(i => fullY[i]) };
    });

  }).finally(() => { if (pageLoader) pageLoader.style.display = 'none'; });
  ...
  function enablePQRSTEditing(plotElement, data, pqrst) {
    ...
    plotElement.on('plotly_click', event => {
      ...
      if (!['p','q','r','s','t'].includes(key)) return;
      selectedPoint = { key: key === 'r' ? 'r_peaks' : `${key}_points`, pointIndex: pt.pointIndex, traceIndex: pt.curveNumber };
    });
    ...
    plotElement.addEventListener('mousemove', e => {
      ...
      Plotly.restyle(plotElement, { x: [updated[selectedPoint.key].map(i => data.x[i])], y: [updated[selectedPoint.key].map(i => data.y[i])] }, [selectedPoint.traceIndex]);
    });
