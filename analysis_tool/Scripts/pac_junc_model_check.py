...
# #    csv_name = 'pac_ jun_testing_model_result.csv'
# #    with open(save_path + csv_name, 'a') as csv_file:
# #        np.savetxt(csv_file,
# #                   np.array([local_name, arrhythmia_result['jr_label'], 'II, III, aVF, v5', ', '.join(map(str, arrhythmia_result['pac_index']))]).reshape(1, -1),
# #                   delimiter=',', fmt='%s', comments='')
    
#     footer_text = f"{arrhythmia_result['jr_label']}"
...
#     # Update label_counts
#     labels = arrhythmia_result.get("jr_label", "").lower().split(";")
#     for label in labels:
#         label = label.strip()
#         if label:
#             if local_name not in label_counts:
...
#     pac_load_model = load_tflite_model(os.getenv("PAC"))
#     frequency = 200
#     label_counts = {}
#     hr_counts = {}
#     pdf = PDF()
