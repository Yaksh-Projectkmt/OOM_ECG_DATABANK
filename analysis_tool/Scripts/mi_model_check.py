...
def prediction_model_mi(input_arr, let_inf_moedel):
...
def check_st_model(low_ecg_signal,temp_mi_img_path, let_inf_moedel):
    label = 'Abnormal'
    for i in glob.glob(temp_mi_img_path + "/*.jpg"):
        os.remove(i)
    # try:
    randome_number = random.randint(200000, 1000000)
    plt.plot(low_ecg_signal)
    plt.axis("off")
    plt.savefig(f"{temp_mi_img_path}/{randome_number}.jpg")
    aq = cv2.imread(f"{temp_mi_img_path}/{randome_number}.jpg")
    aq = cv2.resize(aq, (1080, 460))
    cv2.imwrite(f"{temp_mi_img_path}/{randome_number}.jpg", aq)
    plt.close()
    stdep = []
    stele = []
    abn_t = []
    ...
    files = sorted(glob.glob(temp_mi_img_path+ "/*.jpg"), key=extract_number)
    for pvcfilename in files:
        predictions, ids = prediction_model_mi(pvcfilename, let_inf_moedel)
        label = "Normal"
...
def check_mi_model(all_leads_data, temp_mi_img_path, let_inf_moedel):
    fs=200
    mi_result = 'Abnormal'
    all_lead_det_data = {}
    t_abn = []
    ...
    try:
        for lead in all_leads_data.columns:
            ...
            if lead in ['II', 'III', 'aVF', 'I', 'aVL', 'v5']:
                ...
                mi_model_result = check_st_model(lowpass_signal, temp_mi_img_path, let_inf_moedel)
                lead_data['mi_result'] = mi_model_result
        ...
        if len(all_lead_det_data.keys()) > 1:
            ...
            repeated_elements = [item for item, count in counts.items() if count >= 3 and item=="TAB"]
            mi_t_lab = ' '.join(repeated_elements)
            print(mi_t_lab, "====================mi_t_lab")
            if mi_t_lab == "TAB" and mi_result != "Inferior STEMI" and mi_result != "Lateral STEMI":
                mi_result = "T_wave_Abnormality"
            # elif mi_t_lab != 'Normal' and "TAB" in t_abn:
            #     mi_result = "T_wave_Abnormality"
        else:
            if all_lead_det_data['II']['mi_result'] == 'TAB':
                mi_result = "T_wave_Abnormality"
            else:
                mi_result = all_lead_det_data['II']['mi_result']
    except Exception as e:
        is_error = True
        print('MI detection error: ', e)
    result_dic = {
        'mi_result': mi_result,
        'all_lead_det_data': all_lead_det_data,
        'hr': hr_counts,
        'is_error': is_error
    }
    return result_dic
...
# def plotting(mi_detector,all_leads_data, save_path, local_name,is_lead_for, pdf, label_counts,hr_counts):
...
#     hr_value = mi_detector.get("hr")
#     if hr_value is not None:
#         if local_name not in label_counts:
#             label_counts[local_name] = {"hr_list": []}
#         label_counts[local_name]["hr_list"].append(hr_value)
    
#     # plt.suptitle(f"{local_name}_{mi_label}")
# #    csv_name = 'mi_testing_model_result.csv'
# #
# #    with open(save_path + csv_name, 'a') as csv:
# #        np.savetxt(csv, np.array([local_name, mi_detector['mi_result'],'II, III, aVF, v5']).reshape(1,-1), delimiter= ',', fmt = '%s', comments ='')
    
#     footer_text = f"{mi_detector['mi_result']}"
...
# # Update label_counts
#     labels = mi_detector.get("mi_result", "").lower().split(";")
#     for label in labels:
#         label = label.strip()
#         if label:
#             if local_name not in label_counts:
...
#     temp_mi_img_path =os.path.join( temp_image,'ST_model_temp_img')
#     os.makedirs(temp_mi_img_path, exist_ok=True)
#     let_inf_moedel =load_tflite_model(os.getenv("MI"))# add your model path
#     frequency = 200
#     label_counts = {}
#     hr_counts = {}
#     pdf = PDF()
...
#             mi_detector = check_mi_model(ecg_data, temp_mi_img_path, let_inf_moedel)
#             plotting(mi_detector ,ecg_data, save_result, local_name,is_lead_for,pdf, label_counts,hr_counts)
#             i += steps
...
def plotting(mi_detector,all_leads_data,local_name,is_lead_for, pdf, label_counts,hr_counts):
    if is_lead_for == '7_Lead':
        limb_leads = ['I', 'III', 'aVL', 'v5']
        chest_leads = ['II', 'aVR', 'aVF']
    else:
...
def model_check_for_ecg_data(file_path, is_lead_for):
    ...
    for fn in glob.glob(file_path):
        ...
        while i < len(all_lead_data):
            ...
            mi_detector = check_mi_model(ecg_data, temp_mi_img_path, let_inf_moedel)
            buf=plotting(mi_detector ,ecg_data,local_name,is_lead_for,None, label_counts,hr_counts)
