...
def prediction_model_PAC(input_arr, pac_load_model, target_shape=[224, 224], class_name=True):
...
class PACDetection:
    def __init__(self,get_signal, fs, is_lead = "lead_2"):
        self.get_signal = get_signal
        self.fs = fs
        self.is_lead = is_lead

    def get_pac_data(self, pac_load_model):
        all_lead_pac_data = {}
        ...
        temp_img_pac = "D:/OneDrive - KALI MEDTECH PRIVATE LIMITED/Avani/ECG/ECG_7_lead_detection/pac_img_plot/"
        for i in glob.glob(temp_img_pac+"/*.jpg"):
            os.remove(i)
        for lead in all_lead_data.keys():
            if lead in ['II', 'III', 'aVF', 'v5']: #['I', 'II', 'III', 'aVR', 'aVL', 'aVF', 'v1', 'v2', 'v3', 'v4', 'v5', 'v6']:
                ...
                apeds = []
                r_index = detect_beats(lowpass_signal,200)
                ...
                if Average(variations)<0.20:
                    ...
                    try:
                        pqrst_data = pqrst_detection(base_signal, fs=200, thres=0.37, lp_thres=0.1, rr_thres = 0.15).get_data()
                        ...
                        for i in range(len(r_index) - 1):
                            ...
                            ax.axis(False)
                            # plt.savefig(f"{temp_img_pac}{r_index[i]}.jpg")
                            fig.canvas.draw()
                            ...
                            predictions,ids = prediction_model_PAC(tensor_image, pac_load_model)
                            ...
                            elif (str(ids) == "Junctional" and float(predictions[1])>0.80) and R_label == 'Regular' and lead in ['II', 'III', 'aVF']:
                                junc_union.append(1)
        ...
        if len(all_lead_pac_data.keys()) > 1:
            all_pac_lists = [all_lead_pac_data[lead]['updated_union'] for lead in all_lead_pac_data.keys()]        
            max_list_ind = max(map(lambda x:len(x),all_pac_lists))
            all_pac_list_final = [inner_list + [0] * (max_list_ind - len(inner_list)) for inner_list in all_pac_lists]
            combine_pvcs = [1 if sum(sublist[i] for sublist in all_pac_list_final) >= 2 else 0 for i in range(len(all_pac_list_final[0]))]
            actaulPAC = combine_pvcs
            temp_lead = max(all_lead_pac_data, key=lambda k: len(all_lead_pac_data[k]))
            for_pac = all_lead_pac_data[temp_lead]['forPAC']
            hr_counts = all_lead_pac_data[temp_lead]['hr']
            pac_index = [all_lead_pac_data[temp_lead]['r_index'][i] for i in range(len(actaulPAC)) if actaulPAC[i] == 1]
            pac_predict = all_lead_pac_data[temp_lead]['pac_detect']
            keys_with_junc_detect = list(filter(lambda key: all_lead_pac_data[key]['junc_detect'], all_lead_pac_data.keys()))
            if len(keys_with_junc_detect) != 0:
                jr_predict = all_lead_pac_data[keys_with_junc_detect[0]]['junc_detect']
            else:
                jr_predict = []
            jun_labels_all = list(map(lambda x: x['junctional_label'], all_lead_pac_data.values()))
            flat_list = []
            for element in jun_labels_all:
            ...
            jr_label = ' '.join(repeated_elements)

        else:
            actaulPAC = all_lead_pac_data['II']['updated_union']
            for_pac = all_lead_pac_data['II']['forPAC']
            hr_counts = all_lead_pac_data['II']['hr']
            pac_index = [all_lead_pac_data['II']['r_index'][i] for i in range(len(actaulPAC)) if actaulPAC[i] == 1]
            jr_label = all_lead_pac_data['II']['junctional_label']
            jr_predict = all_lead_pac_data['II']['junc_detect']
            pac_predict = all_lead_pac_data['II']['pac_detect']
        return actaulPAC, jr_label, for_pac, hr_counts, pac_index, jr_predict, pac_predict
...
def check_arrhythmia(all_lead_data,pac_load_model,  date_time):
    fa = 200
    pac_label = 'Abnormal'
    pac_index = []
    hr_counts = None
    is_error = False
    try:
        get_pac_detection = PACDetection(all_lead_data, fa,  is_lead= 'lead_7')
        updated_union, jr_label, forPAC, hr_counts, pac_index, jr_predict, pac_predict = get_pac_detection.get_pac_data(pac_load_model)
        # bigem_count, Trigem_count, Quadgem_count, pac_c_count, pac_t_count, at_count =  get_pac_detection.pac_count_find(updated_union, hr_counts)
        pac_data = get_pac_detection.pac_count_find(updated_union, hr_counts)
        ...
        is_error = True
        print('\033[93m' + f"Model prediction Error:{e}" + '\033[0m')
    result_dic = {
        'pac_label': pac_label,
        'pac_index': pac_index,
        'jr_label': jr_label,
        'jr_predict_index': jr_predict,
        'pac_predict_index': pac_predict,
        'hr': hr_counts,
        'is_error': is_error
    }
...
# #
# #    csv_name = 'pac_testing_model_result.csv'
# #    with open(save_path + csv_name, 'a') as csv_file:
# #        np.savetxt(csv_file,
# #                   np.array([local_name, arrhythmia_result['pac_label'], 'II, III, aVF, v5', ', '.join(map(str, arrhythmia_result['pac_index']))]).reshape(1, -1), 
# #                   delimiter=',', fmt='%s', comments='')
        
#     footer_text = f"{arrhythmia_result['pac_label']}"
...
#     # Update label_counts
#     labels = arrhythmia_result.get("pac_label", "").lower().split(";")
#     for label in labels:
#         label = label.strip()
#         if label:
#             if local_name not in label_counts:
...
#     model_path=os.getenv("PAC")
#     pac_load_model = load_tflite_model(model_path)
#     frequency = 200
#     label_counts = {}
#     hr_counts = {}
#     pdf = PDF()
...
#             arrhythmia_detector = check_arrhythmia(ecg_data, pac_load_model, date_time)
